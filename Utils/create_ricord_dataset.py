...
"""
python : 3.6.1
xldr: support to handle xls (excel) files
gdcm: dependency library
take from: https://github.com/lindawangg/COVID-Net/blob/master/create_ricord_dataset/create_ricord_dataset.ipynb
"""
import os
import cv2
import glob
...
import numpy as np
import pandas as pd
from routes import covid_raw_folder
...
ricord_dir = 'MIDRC-RICORD-1C'
ricord_meta_file = 'MIDRC_RICORD_1c_ClinicalDataJan132021.xls'
out_dir = 'out_dir_folder'
ricord_set_file = 'create_ricord_dataset/ricord_data_set.txt'


def generate_ricord_images(since=0):
    ...
    for mrn, date, desc, uid in metadata:
        ...
        for i, dcm_file in enumerate(dcm_files):
            ...
            # 16/05/21 create my custom custom_name
            since = since + 1
            custom_name = 'covid-' + str(since) + '.png'
            out_path = os.path.join(out_dir, custom_name)

            # Save image
            cv2.imwrite(out_path, image)
            file_count += 1
    print('Created ROCORD {} files'.format(file_count))


def main():
    generate_ricord_images()
